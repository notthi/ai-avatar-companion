...
// AR配置時のサイズ倍率(実寸=1.0に対する比率)
const AR_SCALES = { normal: 0.75, fairy: 0.1 } as const
...
export function VrmAvatar({ state, getLevel, getSpeaking, emotion, spinSignal, dancing, onPoke }: Props) {
  ...
  // AR配置時のサイズ: 標準(実寸の75%)/ 妖精(10%)。選択は端末に保存
  const [arScaleMode, setArScaleMode] = useState<'normal' | 'fairy'>(
    () => (localStorage.getItem('ar-scale') === 'fairy' ? 'fairy' : 'normal'),
  )
  const arScaleRef = useRef(arScaleMode)
  arScaleRef.current = arScaleMode
  const toggleArScale = () => {
    setArScaleMode((m) => {
      const next = m === 'normal' ? 'fairy' : 'normal'
      localStorage.setItem('ar-scale', next)
      return next
    })
  }
  ...
  useEffect(() => {
    ...
    const animate = (_time?: number, frame?: XRFrame) => {
      ...
      if (vrm) {
        ...
        const arScaleTarget = arPlace.pos ? AR_SCALES[arScaleRef.current] : 1
      ...
      {arActive && (
        <>
          <p className="absolute top-2 left-2 right-20 z-10 text-xs text-white/80 bg-black/40 rounded-xl px-3 py-1.5 pointer-events-none">
            床や机にスマホをかざして、リングが出た場所をタップで配置(もう一度タップで移動)
          </p>
          <button
            onClick={toggleArScale}
